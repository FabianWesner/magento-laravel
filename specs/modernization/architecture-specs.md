...
| PHP runtime | How the target runtime stays on the latest stable PHP while the legacy smoke runtime remains isolated. | PHP version policy, Composer platform config, CI matrix, extension list, upgrade cadence. | `php -v`, Composer platform checks, CI build, Laravel Boost install. |
...
- Laravel target code runs on the latest stable PHP; legacy Magento PHP `7.4` is allowed only inside isolated baseline verification tooling.
...
- Target the latest stable PHP for all Laravel code and Composer dependencies; as of 2026-05-18 this means PHP `8.5.x`.
