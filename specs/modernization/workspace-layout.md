...
The current root checkout is OpenMage core/source only. The local smoke baseline now uses a separate ignored Magento CE `1.9.4.5` mirror checkout under `.localdev/magento-mirror-1.9.4.5`. The modernization work still needs a two-repository workspace once the real project repository is available.
...
|-- magento-core-1.9.4.5/ # Magento CE 1.9.4.5 mirror checkout
...
| --- | --- | --- |
| OpenMage core checkout | Source inventory and previous preparation context | Present. |
| Magento CE `1.9.4.5` mirror checkout | Official Magento 1 runtime baseline and smoke verification | Present under ignored `.localdev/`. |
...
1. Keep `magento-lts/` intact as the preparation repository until the project repository path is known.
2. Keep `.localdev/magento-mirror-1.9.4.5` as the ignored Magento CE baseline for local smoke checks.
...
php /path/to/modernization-workspace/magento-lts/dev/modernization/inventory.php --root=/path/to/modernization-workspace/project --format=markdown
DB_DSN='mysql:host=127.0.0.1;dbname=<project_db>' DB_USER=<user> DB_PASS=<pass> php /path/to/modernization-workspace/magento-lts/dev/modernization/schema-report.php --format=markdown
```
