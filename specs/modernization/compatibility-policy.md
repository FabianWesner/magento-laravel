...
- Laravel target code runs on the latest stable PHP; legacy Magento PHP `7.4` is allowed only for isolated baseline verification.
...
| PHP runtime | Upgrade target Laravel runtime to latest stable PHP; isolate legacy Magento PHP `7.4` smoke runtime | `php -v`, Composer platform check, CI matrix, Laravel Boost install. |
