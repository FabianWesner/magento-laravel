...
| 4 | Feature inventory | Expanded | Use `magento-feature-catalog.md` as the canonical feature list and complete project-specific storefront, admin, API, cron, integration, and reporting inventory rows. |
...
| 9 | Visual baseline | Expanded | Capture every screen in `ui-screen-inventory.md` across supported breakpoints, roles, states, fixtures, and side-by-side runtimes before UI migration. |
| 10 | Performance budgets | Drafted | Capture current p50/p95, query counts, memory, cache behavior, and set budgets per journey. |
| 11 | Data fixture strategy | Expanded | Version fixtures or sanitized dumps for minimal, seed, all product types, promotions, tax, shipping, payment, multistore, EAV, sales, permissions, API, cron, reports, media, and scale data. |
...
- The canonical Magento feature catalog, UI screen inventory, complex feature reverse-engineering specs, and demo fixture matrix are complete.
