...
Changed:
- Added the `/_modernization/admin/promotions` Livewire workbench to inspect deterministic cart price rule, catalog price rule, coupon, coupon report, and promotion problem rows.
- Added `promotion_rule`, `catalog_price_rule`, `promotion_coupon`, and `promotion_report` domain features and deterministic facts across default and DE store views.
- Covered active and invalid cart rules, scheduled, expired, and stale catalog rules, autogenerated and exhausted coupons, coupon report aggregation, and disabled read-only inspection actions.
- Added the admin promotions route, CSS asset route, wrapper Blade view, responsive CSS, focused PHPUnit route/Livewire/domain coverage, modernization module manifest references, and ignored local admin-promotions Playwright screenshots.
- Updated tasklist, open-issues, backlog, and reasoning tracking after Chrome verification and the normal gate.

Verified:
- Required Laravel docs lookup was attempted before code changes, but the available local docs command tried to open the remote Laravel docs URL from the sandbox and could not complete.
- Parallel subagents supplied legacy Magento promotion/cart-rule/catalog-rule/coupon/report constraints, Laravel workbench implementation patterns, and project rule constraints before finalizing the slice.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l app/Livewire/AdminPromotionsWorkbench.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l database/seeders/DomainFactSeeder.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l tests/Feature/ModernizationAdminPromotionsRouteTest.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l tests/Feature/DomainFoundationTest.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l routes/web.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l app/Modernization/Domain/DomainCatalog.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l config/modernization.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH vendor/bin/pint --dirty --format agent` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php artisan test --compact tests/Feature/ModernizationAdminPromotionsRouteTest.php tests/Feature/DomainFoundationTest.php tests/Feature/CommerceFoundationTest.php tests/Feature/ReportFoundationTest.php tests/Feature/CronJobSchedulerTest.php tests/Feature/ModernizationModuleRegistryTest.php` passed with 45 tests and 1174 assertions.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php artisan db:seed --class=DomainFactSeeder --no-interaction` seeded local browser data.
- `curl -I http://magento-lts.test/_modernization/admin/promotions` returned HTTP 200.
- `curl -I http://magento-lts.test/_modernization/assets/admin-promotions.css` returned HTTP 200.
- Chrome/Playwright desktop verification passed for promotion summary counts, invalid cart rule filtering, stale catalog rule filtering, exhausted coupon filtering, DE store-view scoping, coupon report aggregation rows, aggregated problem rows, empty query state, denied role state, disabled read-only actions, and clean current console output.
- Chrome/Playwright mobile verification passed for responsive single-column controls, wrapped summary counts, readable problem cards, disabled actions, and clean current console output.
- Screenshots were captured as `admin-promotions-desktop-2026-05-19.png` and `admin-promotions-mobile-2026-05-19.png`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH bash dev/modernization/gate.sh` passed in normal no-DB mode, with Laravel tests passing 173 tests and 2360 assertions, fixture coverage/schema skipped because `DB_DSN` is unset, and Docusaurus browser smoke skipped by sandbox bind restrictions.

Blocked:
- Laravel docs lookup remains blocked in this environment.
- This is a local modernization workbench slice only; final promotion parity still needs real project promotion, coupon, catalog price rule, report, and price/index fixtures; write-operation characterization; coupon generation collision behavior; admin ACL integration; hosted CI; manual acceptance; and final Magento/Laravel screenshot evidence.

Next:
- Commit this verified admin promotions implementation slice, then continue implementation-first on the next browser-verifiable Magento domain slice.
...
Changed:
- Added the `/_modernization/admin/customer-management` Livewire workbench to inspect deterministic customer grid, address book, wishlist, compare, review moderation, tag moderation, and problem rows from existing domain facts.
- Added the admin customer route, CSS asset route, wrapper Blade view, responsive CSS, focused PHPUnit route/Livewire coverage, modernization module manifest references, and ignored local admin-customer Playwright screenshots.
- Kept the slice read-only after the legacy scan confirmed customer saves, address updates/deletes, newsletter status changes, group assignment, password changes, mass actions, emails, wishlist deletes/configures, review edits, and tag edits are high-risk before final fixtures and auth cutover.
- Updated tasklist, open-issues, backlog, and reasoning tracking after Chrome verification.

Verified:
- Required Laravel docs lookup was attempted before code changes, but the available local docs command tried to open the remote Laravel docs URL from the sandbox and could not complete.
- Parallel subagents supplied Laravel workbench pattern review and legacy Magento admin customer/account/address/activity/moderation behavior context before finalizing the slice.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l app/Livewire/AdminCustomerWorkbench.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l routes/web.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l tests/Feature/ModernizationAdminCustomerRouteTest.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH vendor/bin/pint --dirty --format agent` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php artisan test --compact tests/Feature/DomainFoundationTest.php tests/Feature/ModernizationAdminCustomerRouteTest.php tests/Feature/ModernizationModuleRegistryTest.php` passed with 24 tests and 794 assertions.
- `curl -I http://magento-lts.test/_modernization/admin/customer-management` returned HTTP 200.
- `curl -I http://magento-lts.test/_modernization/assets/admin-customer.css` returned HTTP 200.
- Chrome/Playwright desktop verification passed for initial customer summary, address section, wishlist/compare activity filtering, pending review/tag moderation filtering, problem rows, DE store-view row, empty query state, denied role, disabled read-only actions, and clean current console output.
- Chrome/Playwright mobile verification passed for responsive single-column controls, visible summary counts, DE problem empty state, disabled actions, and clean current console output.
- Screenshots were captured as `admin-customer-desktop-2026-05-19.png` and `admin-customer-mobile-2026-05-19.png`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php dev/modernization/markdown-check.php` passed for 83 files.
- `git diff --check` passed.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH bash dev/modernization/gate.sh` passed in normal no-DB mode, with Laravel tests passing 150 tests and 1915 assertions, fixture coverage/schema skipped because `DB_DSN` is unset, and Docusaurus browser smoke skipped by sandbox bind restrictions.

Blocked:
- Laravel docs lookup remains blocked in this environment.
- This is a local modernization workbench slice only; final admin customer parity still needs real project customer fixtures, multi-website duplicate-email cases, full EAV account/address forms, order/cart/newsletter queue fixtures, admin ACL integration, hosted CI, manual acceptance, and final Magento/Laravel screenshot evidence.

Next:
- Continue implementation-first on the next browser-verifiable Magento domain slice.

## 2026-05-19 16:11 CEST - Admin Catalog Management Workbench Implementation

Changed:
- Added the `/_modernization/admin/catalog-management` Livewire workbench to inspect deterministic product, category, EAV-like attribute, media gallery, downloadable product, and problem rows from existing domain facts.
- Added the admin catalog route, CSS asset route, wrapper Blade view, responsive CSS, focused PHPUnit route/Livewire coverage, modernization module manifest references, and ignored local admin-catalog Playwright screenshots.
- Kept the slice read-only after the legacy scan confirmed product/category saves, mass actions, category moves, media uploads, downloadable uploads, URL rewrite updates, cache/index side effects, and EAV writes are high-risk before final fixtures and auth cutover.
- Added UI and normalizer support for legacy product-type/status filters including grouped, bundle, virtual, disabled, inactive, out-of-stock, shareable, missing media, and problem states.
- Updated tasklist, open-issues, backlog, and reasoning tracking after Chrome verification.

Verified:
- Required Laravel docs lookup was attempted before code changes, but the available local docs command tried to open the remote Laravel docs URL from the sandbox and could not complete.
- Parallel subagents supplied Laravel workbench pattern review and legacy Magento admin catalog/product/category/media/downloadable behavior context before finalizing the slice.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l app/Livewire/AdminCatalogWorkbench.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l routes/web.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php -l tests/Feature/ModernizationAdminCatalogRouteTest.php` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH vendor/bin/pint --dirty --format agent` passed from `laravel/`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php artisan test --compact tests/Feature/DomainFoundationTest.php tests/Feature/ModernizationAdminCatalogRouteTest.php tests/Feature/ModernizationModuleRegistryTest.php` passed with 24 tests and 794 assertions.
- `curl -I http://magento-lts.test/_modernization/admin/catalog-management` returned HTTP 200.
- `curl -I http://magento-lts.test/_modernization/assets/admin-catalog.css` returned HTTP 200.
- Chrome/Playwright desktop verification passed for initial product counts, categories, configurable attributes, missing-media filtering, downloads, problems, DE store-view rows, empty query state, denied role, disabled read-only actions, grouped filter normalization, and clean current console output.
- Chrome/Playwright mobile verification passed for responsive single-column controls, visible summary counts, DE problem filtering, disabled actions, and clean current console output.
- Screenshots were captured as `admin-catalog-desktop-2026-05-19.png` and `admin-catalog-mobile-2026-05-19.png`.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH php dev/modernization/markdown-check.php` passed for 83 files.
- `git diff --check` passed.
- `env PATH=/private/tmp/magento-lts-php85-bin:$PATH bash dev/modernization/gate.sh` passed in normal no-DB mode, with Laravel tests passing 146 tests and 1868 assertions, fixture coverage/schema skipped because `DB_DSN` is unset, and Docusaurus browser smoke skipped by sandbox bind restrictions.

Blocked:
- Laravel docs lookup remains blocked in this environment.
- This is a local modernization workbench slice only; final admin product/category/EAV parity still needs real project catalog fixtures, real category tree roots, full product-type coverage, EAV backend/source validation artifacts, stock/media/downloadable write characterization, admin ACL integration, hosted CI, manual acceptance, and final Magento/Laravel screenshot evidence.
