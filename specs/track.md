...
I implemented this as a read-only workbench under `/_modernization/admin/promotions`. It exposes active and invalid cart rules, scheduled, expired, and stale catalog rules, autogenerated and exhausted coupons, coupon report aggregation rows, DE store-view rows, empty states, denied role behavior, and disabled inspection buttons. The important result is that promotion behavior can now be inspected and discussed in Chrome without changing storefront prices or coupon inventory.

The legacy scan shaped the scope:

- Cart price rule saves mutate conditions, labels, websites, customer groups, coupon metadata, and generated-coupon settings, so the workbench shows rule health and coupon outcomes without persisting changes.
- Catalog price rule apply operations write rule-product and price index data, invalidate cache, and trigger price reindexing concerns, so stale application is a first-class diagnostic instead of an action button.
- Coupon generation inserts many rows and can alter requested lengths when collision probability is too high, so generated and exhausted coupon states are visible but generation remains disabled.
- Coupon reports depend on aggregate refresh jobs and report tables, so the workbench shows aggregation freshness instead of invoking report statistics refresh paths.
- Problem rows span invalid conditions, stale catalog application, and exhausted coupons because promotion support work usually crosses those boundaries.
- The `sales`, `catalog`, and read-only diagnostic roles are normalized locally so role behavior can be exercised without weakening production admin boundaries.

This creates browser-verifiable local evidence for `AD-008`, `CB-003`, `CB-004`, `CB-005`, `CJ-014`, `CJ-015`, and related report work, but it is not promotion cutover. Real promotion fixtures, coupon collision characterization, catalog rule price/index comparisons, order-total impact tests, admin ACL integration, hosted CI, manual acceptance, production runbooks, and final Magento/Laravel screenshot evidence remain open.
